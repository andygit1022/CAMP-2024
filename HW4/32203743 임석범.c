...
#define CACHE_LINE_SIZE 4    // 캐시 라인 데이터 크기 (4 bytes)
...
int cache_check_hit(uint32_t address, uint32_t* set_index, uint32_t* tag) {
    cache_access++;
...
int cache_handle_miss(uint32_t address, uint32_t set_index, uint32_t tag) {
...
int cache_find_empty_line(CacheSet* set) {
    for (int i = 0; i < CACHE_ASSOC; i++) {
        if (set->lines[i].valid == 0) {
            return i; // 빈 캐시 라인의 인덱스 반환
        }
    }
    return -1; // 빈 캐시 라인이 없는 경우
}


void log_cache_event(const char* event, uint32_t address, uint32_t tag, uint32_t set_index, int lru_index) {
    printf("[Cache Event] %s - Address: 0x%08x, Tag: 0x%08x, Set Index: %d, LRU Index: %d\n", event, address, tag, set_index, lru_index);
}
...
void fetch(Registers* r, uint8_t* mem, if_id* if_id) {
    ...
    uint32_t address = r->pc;
    uint32_t set_index, tag;

    // 캐시 히트 여부 확인
    int hit_index = cache_check_hit(address, &set_index, &tag);
    ...
    if (hit_index != -1) { // 캐시 히트
        ...
        log_cache_event("Fetch Hit", address, tag, set_index, hit_index);
    }
    else { // 캐시 미스
     // 메모리에서 명령어를 읽어옴
        int lru_index = find_lru_index(&cache.sets[set_index]);
        CacheSet* set = &cache.sets[set_index];
        CacheLine* line = &set->lines[lru_index];

        // LRU 알고리즘을 사용하여 교체할 캐시 라인을 선택
        if (line->dirty) {
            uint32_t old_address = (line->tag * CACHE_SET_SIZE + set_index) * CACHE_LINE_SIZE;
            // (((line->tag << 8) | set_index) << 2) 과 같음.
            for (int i = 0; i < CACHE_LINE_SIZE; i++) {
                Memory[old_address + i] = line->data[i];
            }
        }
        ...
        // 메모리에서 새로운 데이터 읽어오기
        uint32_t new_address = address - (address % CACHE_LINE_SIZE);
        for (int i = 0; i < CACHE_LINE_SIZE; i++) {
            line->data[i] = Memory[new_address + i];
        }
        ...
        update_lru(set, lru_index);
        log_cache_event("Fetch Miss", address, tag, set_index, lru_index);
    }
...
void mem_access(uint8_t* mem, ex_mem* ex_mem, mem_wb* mem_wb) {
    ...
    uint32_t address = ex_mem->alu_res;
    uint32_t set_index, tag;
    int hit_index;

    if (ex_mem->ctrl->mem_read == 1) { // lw
        lw_count++;
        hit_index = cache_check_hit(address, &set_index, &tag);
        ...
        if (hit_index != -1) { // 캐시 히트
            ...
            mem_wb->mem_read = 0;
            for (int i = 0; i < 4; i++) {
                mem_wb->mem_read |= line->data[i] << (8 * (3 - i));
            }

            // LRU 업데이트
            update_lru(set, hit_index);
            log_cache_event("Memory Access Hit", address, tag, set_index, hit_index);
        }
        else { // 캐시 미스
         // 메모리에서 데이터 읽어오기
            int lru_index = cache_handle_miss(address, set_index, tag);
            CacheSet* set = &cache.sets[set_index];
            CacheLine* line = &set->lines[lru_index];

            mem_wb->mem_read = 0;
            for (int i = 0; i < 4; i++) {
                mem_wb->mem_read |= mem[address + i] << (8 * (3 - i));
                line->data[i] = mem[address + i];
            }
            log_cache_event("Memory Access Miss", address, tag, set_index, lru_index);
        }
    }
        ...
        if (hit_index != -1) { // 캐시 히트
            ...
            // 캐시 세트가 가득 차있는지 검사
            int lru_index = cache_find_empty_line(set);
            if (lru_index == -1) {
                lru_index = find_lru_index(set);
            }

            // 데이터 저장
            for (int i = 0; i < 4; i++) {
                line->data[i] = (ex_mem->mem_val >> (8 * (3 - i))) & 0xFF;
            }
...
int main(void) {
    ...
    FILE* file = fopen("input4.bin", "rb"); // "filename.bin" => filename 변경해서 다른파일 넣기.
    if (!file) {
